...
async def main():
    ...
    logger.info("=" * 80)
    logger.info(
        "Propósito: Verificar que el sistema soporta múltiples scrapers independientes"
    )
    logger.info("=" * 80)
    ...
    try:
        count_clarin = await sync_to_async(
            NewsArticleModel.objects.filter(fuente="Clarín").count
        )()
        count_pagina12 = await sync_to_async(
            NewsArticleModel.objects.filter(fuente="Página 12").count
        )()
        count_lanacion = await sync_to_async(
    ...
    try:
        count_clarin_after = await sync_to_async(
            NewsArticleModel.objects.filter(fuente="Clarín").count
        )()
        count_pagina12_after = await sync_to_async(
            NewsArticleModel.objects.filter(fuente="Página 12").count
        )()
        count_lanacion_after = await sync_to_async(
            NewsArticleModel.objects.filter(fuente="La Nación").count
        )()
        ...
        logger.info(
            f"   Clarín: {count_clarin_after} artículos (+{count_clarin_after - count_clarin})"
        )
        logger.info(
            f"   Página 12: {count_pagina12_after} artículos (+{count_pagina12_after - count_pagina12})"
        )
        logger.info(
            f"   La Nación: {count_lanacion_after} artículos (+{count_lanacion_after - count_lanacion})"
        )
    ...
    if fuentes_con_datos >= 2:
        ...
        logger.info(
            "   Los scrapers pueden ejecutarse de forma independiente y persistir"
        )
    ...
    for fuente in ["Clarín", "Página 12", "La Nación"]:
        articles = await sync_to_async(list)(
            NewsArticleModel.objects.filter(fuente=fuente).order_by(
                "-fecha_publicacion"
            )[:3]
        )
        if articles:
            count_fuente = await sync_to_async(
                NewsArticleModel.objects.filter(fuente=fuente).count
            )()
