        ...
        try:
            ...
            for dto in article_dtos:
                try:
                    ...
                    logger.info(f"Artículo guardado exitosamente: {saved_article.titulo[:60]}...")
                    
                except Exception as e:
                    logger.error(f"Error procesando artículo {dto.url}: {e}", exc_info=True)
                    continue
            ...
            logger.error(f"Error en caso de uso ScrapeAndPersistArticles: {e}", exc_info=True)
            raise
