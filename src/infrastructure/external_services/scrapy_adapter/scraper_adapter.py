...
class ScrapyAdapter(IScraperPort):
        ...
        try:
            ...
            def item_scraped(item, response, spider):
                try:
                    ...
                    logger.info(f"Artículo procesado: {article.titulo[:50]}... - Fuente: {article.fuente}")
            ...
            logger.info(f"Scraping completado. Total artículos: {len(self.scraped_articles)}")
    ...
    def _get_scrapy_settings(self):
        from .settings import (
            BOT_NAME, ROBOTSTXT_OBEY, CONCURRENT_REQUESTS, DOWNLOAD_DELAY,
            CONCURRENT_REQUESTS_PER_DOMAIN, COOKIES_ENABLED, DEFAULT_REQUEST_HEADERS,
            ITEM_PIPELINES, AUTOTHROTTLE_ENABLED, AUTOTHROTTLE_START_DELAY,
