...
class ClarinSpider(BaseNewsSpider):
    ...
    def parse_article(self, response):
        try:
            ...
            paragraphs = response.css('div.body-nota p::text, div.StyledParagraph p::text, article p::text').getall()
            contenido = ' '.join(paragraphs)
            
            if not contenido:
                contenido_div = response.css('div.body-nota, div.StyledParagraph, article').get()
                if contenido_div:
