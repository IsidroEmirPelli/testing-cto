#!/usr/bin/env python3
"""
Script de prueba para el scraper de Clarín.
...
async def main():
    ...
    logger.info("="*80)
    logger.info("TEST DEL SCRAPER DE CLARÍN")
    logger.info("="*80)
    ...
    try:
        ...
        if result['articles']:
            ...
            for i, article in enumerate(result['articles'][:5], 1):
                ...
                logger.info(f"  Contenido (primeros 150 chars): {article.contenido[:150]}...")
        ...
        if result['total_new'] >= 10:
            logger.info("✅ CRITERIO CUMPLIDO: Al menos 10 artículos nuevos en la base de datos")
            return_code = 0
        else:
            logger.warning(f"⚠️  CRITERIO NO CUMPLIDO: Solo {result['total_new']} artículos nuevos (se requieren al menos 10)")
            logger.info("    Nota: Puede que haya duplicados. Intenta limpiar la base de datos o ejecutar de nuevo.")
            return_code = 1
