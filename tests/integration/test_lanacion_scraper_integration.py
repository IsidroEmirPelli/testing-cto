...
class TestLaNacionScraperIntegration:
    ...
    async def test_complete_scraping_flow(self):
        ...
        await sync_to_async(NewsArticleModel.objects.filter(fuente="La Nación").delete)()
        ...
        db_count = await sync_to_async(NewsArticleModel.objects.filter(fuente="La Nación").count)()
