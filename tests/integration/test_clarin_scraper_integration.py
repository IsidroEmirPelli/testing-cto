"""
Test de integración para el flujo completo de scraping de Clarín.
"""
import pytest
import os
import django
...
@pytest.mark.asyncio
class TestClarinScraperIntegration:
    """Tests de integración para el scraper de Clarín con la base de datos"""
    
    async def test_complete_scraping_flow(self):
        """Test del flujo completo: scraping → validación → persistencia"""
        # Setup: Limpiar artículos existentes de Clarín
        await sync_to_async(NewsArticleModel.objects.filter(fuente="Clarín").delete)()
        
        # Inicializar componentes
        scraper = ClarinScraper(max_articles=5)  # Solo 5 para tests rápidos
        ...
        db_count = await sync_to_async(NewsArticleModel.objects.filter(fuente="Clarín").count)()
    ...
    async def test_duplicate_detection(self):
        ...
        # Primera ejecución
        scraper = ClarinScraper(max_articles=3)
        repository = DjangoNewsArticleRepository()
        use_case = ScrapeAndPersistArticlesUseCase(scraper, repository)
        ...
        # Puede haber algunos nuevos si Clarín publicó artículos entre ejecuciones
        # pero debe haber detectado al menos algunos duplicados si los artículos son los mismos
        assert result2['total_new'] + result2['total_duplicates'] == result2['total_scraped']
    ...
    async def test_scraper_handles_real_website(self):
        """Test que el scraper puede acceder al sitio real de Clarín"""
        scraper = ClarinScraper(max_articles=2, timeout=30)
        ...
        try:
            ...
            if len(articles) > 0:
                ...
                for article in articles:
                    assert article.fuente == "Clarín"
                    assert article.url.startswith("https://www.clarin.com")
            ...
            pytest.skip("No se pudo acceder al sitio real de Clarín")
